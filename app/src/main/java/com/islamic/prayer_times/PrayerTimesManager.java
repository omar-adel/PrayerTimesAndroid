...
import com.islamic.prayer_times.receivers.AlarmReceiver;
import com.islamic.prayer_times.receivers.BootAndTimeChangeReceiver;
...
public class PrayerTimesManager {
    ...
    public static void updatePrayerTimes(Context context, boolean enableAlarm)
    {
        ...
        // SettingsActivityMy listener calls before setting is committed to shared prefs, so it uses
        // enableAlarm boolean.
    ...
    {
        sAlarmIntent = createAlarmIntent(context);
        AlarmUtil.scheduleAlarm(context,sAlarmIntent,sPrayerTimes.getNext().getTimeInMillis());
        AlarmManager alarmMgr = (AlarmManager)context.getSystemService(ALARM_SERVICE);
        if (null != alarmMgr) {
